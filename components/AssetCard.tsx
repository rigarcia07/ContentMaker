    ...
    <article className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col hover:shadow-lg transition-all duration-300">
      ...
      <div className="p-5 flex-grow space-y-4">
        <div role="tablist" className="flex items-center space-x-3 border-b border-slate-100 pb-2 overflow-x-auto no-scrollbar">
          <button onClick={() => setActiveTab('content')} className={`text-[9px] font-bold uppercase whitespace-nowrap px-1 transition-colors ${activeTab === 'content' ? 'text-orange-600 border-b-2 border-orange-600' : 'text-slate-400 hover:text-slate-600'}`}>Content</button>
          <button onClick={() => setActiveTab('seo')} className={`text-[9px] font-bold uppercase whitespace-nowrap px-1 transition-colors ${activeTab === 'seo' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>SEO</button>
          <button onClick={() => setActiveTab('aeo')} className={`text-[9px] font-bold uppercase whitespace-nowrap px-1 transition-colors ${activeTab === 'aeo' ? 'text-rose-600 border-b-2 border-rose-600' : 'text-slate-400 hover:text-slate-600'}`}>AEO</button>
          <button onClick={() => setActiveTab('accuracy')} className={`text-[9px] font-bold uppercase whitespace-nowrap px-1 transition-colors ${activeTab === 'accuracy' ? 'text-emerald-600 border-b-2 border-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}>Accuracy</button>
        </div>
        ...
        {activeTab === 'seo' && (
          <div className="space-y-3 animate-in fade-in duration-300">
             <div className="bg-indigo-50 p-3 rounded-lg border border-indigo-100 space-y-2">
                ...
                <div className="py-2 border-y border-indigo-200/50">
                  <span className="text-[9px] font-bold text-indigo-400 uppercase block mb-1">SEO Title</span>
                  <p className="text-[11px] text-indigo-800 font-bold">
                    {slice.seoTitle || 'Analyzing keywords for optimized title...'}
                  </p>
                </div>
          ...
          <div className="space-y-3 animate-in fade-in duration-300">
             <div className="bg-rose-50 p-4 rounded-xl border border-rose-100 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-black text-rose-600 uppercase">AEO Snippet</span>
                ...
                <div className="bg-white p-3 rounded-lg border border-rose-200 shadow-sm text-[11px] text-slate-900 leading-relaxed font-medium">
                  {slice.directAnswerSnippet || 'Generating optimized answer...'}
                </div>
                
                <div className="bg-rose-100/70 p-3 rounded-lg border border-rose-200">
                  <div className="flex items-center space-x-1.5 mb-1.5">
                    <div className="w-1.5 h-1.5 bg-rose-600 rounded-full animate-pulse"></div>
                    <span className="text-[9px] font-black text-rose-700 uppercase">AEO STRATEGY PRO-TIP</span>
                  </div>
                  <p className="text-[9px] text-rose-800 leading-relaxed font-medium">
                    Modern Answer Engines (Gemini, Perplexity) favor direct, objective responses under 50 words. Use this snippet to capture voice-search traffic and GenAI summaries.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="flex flex-col bg-white/60 p-2 rounded border border-rose-100/50">
                    ...
                    <span className="text-[10px] font-bold text-rose-900">{slice.aeoMetrics?.voiceReadiness ?? 90}%</span>
                  </div>
                  <div className="flex flex-col bg-white/60 p-2 rounded border border-rose-100/50">
                    <span className="text-[8px] font-bold text-rose-400 uppercase">Structure</span>
                    <span className="text-[10px] font-bold text-rose-900">{slice.aeoMetrics?.snippetStructure || 'Definition'}</span>
                  </div>
                </div>
             </div>
          </div>
          ...
          <div className="space-y-3 animate-in fade-in duration-300">
             <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 space-y-4">
                <div className="flex justify-between items-center border-b border-emerald-200/50 pb-2">
                  ...
                  <span className="text-[10px] font-black text-emerald-800">{slice.consistencyScore ?? 100}% CONSISTENCY</span>
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] font-medium text-emerald-900 leading-relaxed italic bg-white/50 p-2 rounded border border-emerald-100">
                    "{slice.sourceGrounding || 'Asset is directly derived from cornerstone content.'}"
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-2 text-[9px] font-bold text-emerald-800 uppercase">
                  <div className="bg-white/60 p-2 rounded">AODA: {slice.accessibilityAudit?.score ?? 100}%</div>
                  <div className="bg-white/60 p-2 rounded">GEO: {slice.geoMetrics?.citationPotential ?? 85}%</div>
                </div>
             </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between text-xs pt-2">
          <div className="font-semibold text-orange-600 truncate mr-2" aria-label="CTA">{slice.callToAction || 'Learn More'}</div>
          <button onClick={handleCopy} className={`text-[10px] font-bold px-4 py-2 rounded-lg transition-all ${copied ? 'bg-green-600 text-white' : 'bg-slate-900 text-white hover:bg-slate-800 active:scale-95'}`}>
            {copied ? 'Copied' : 'Copy'}
          </button>
        </div>
      </div>
